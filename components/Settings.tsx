...
export const Settings: React.FC<SettingsProps> = ({
  ...
  const handleAddDepartment = async () => {
    if (newDept && !departments.some(d => d.name === newDept)) {
      try {
        const token = localStorage.getItem('asset_track_token');
        ...
        if (res.ok) {
          const added = await res.json();
          setDepartments([...departments, { id: added.id, name: added.name }]);
          setNewDept('');
        }
      } catch (err) {
        ...
        {activeTab === 'taxonomy' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in slide-in-from-bottom-2">
            ...
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border-[3px] border-slate-100 dark:border-slate-800 space-y-8 shadow-sm">
              ...
              <div className="flex flex-wrap gap-3">
                {categories.length > 0 ? categories.map(cat => (
                  <div key={cat.id} className="group flex items-center gap-2 px-5 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-full text-xs font-black uppercase tracking-widest shadow-lg shadow-slate-200 dark:shadow-none transition-all hover:scale-105">
                    {cat.name}
              ...
              <div className="pt-8 border-t border-slate-50 dark:border-slate-800 space-y-6">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Add New Category</p>
            ...
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border-[3px] border-slate-100 dark:border-slate-800 space-y-8 shadow-sm">
              ...
              <div className="flex flex-wrap gap-3">
                {assetLocations.length > 0 ? assetLocations.map(loc => (
                  <div key={loc.id} className="group flex items-center gap-2 px-5 py-2.5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-full text-xs font-black uppercase tracking-widest shadow-lg shadow-slate-200 dark:shadow-none transition-all hover:scale-105">
                    {loc.name}
