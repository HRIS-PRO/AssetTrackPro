              ...
              {columns.map(col => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex items-center justify-center gap-1.5 px-3 py-0 h-10 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all duration-200 text-center leading-tight whitespace-nowrap ${active
                        ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20'
                        : 'bg-slate-50 dark:bg-slate-800 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700'
                      }`}
                  >
                    <span className="truncate">{col.label}</span>
                    {active && <span className="material-symbols-outlined text-[12px] shrink-0 opacity-80">check</span>}
                  </button>
                );
              })}
            </div>
